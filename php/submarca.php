...
class submarca{
    ...
    public function tablaSubmarca(){
        $regsubmarca=mysqli_query($this->conectarBD(), "SELECT * FROM submarca ") or die ("No se realizó con éxito el estado".
        mysqli_error($this->conectarBD()));
        echo "<br><br>";
        echo "<table class='table1'><thead><th>ID Submarca</th>
        <th>Submarca</th><th>ID Marca</th>";
        echo "<tbody>";
        while($submarc=mysqli_fetch_array($regsubmarca)){
            echo "<tr><td>".$submarc[0]."</td><td>".$submarc[1]."</td><td>".$submarc[2]."";
